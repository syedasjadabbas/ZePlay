...
def upgrade() -> None:
    ...
    conn.execute(sa.text("""
        CREATE TABLE IF NOT EXISTS subscription_plans (
            ...
            supports_multi_device BOOLEAN NOT NULL DEFAULT FALSE,
    ...
    if dialect_name == "postgresql":
        conn.execute(sa.text(f"""
            INSERT INTO subscription_plans (id, name, description, max_profiles, supports_4k, supports_multi_device, created_at)
            VALUES
              ('{FREE_PLAN_ID}',    'free',    'Standard streaming with 1 profile.', 1, FALSE, FALSE, '{now}'),
              ('{PREMIUM_PLAN_ID}', 'premium', 'Premium badge, up to 4 profiles, 4K and multi-device ready.', 4, TRUE, TRUE, '{now}')
