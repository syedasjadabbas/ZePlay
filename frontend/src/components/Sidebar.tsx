...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="w-64 bg-brand-background/60 border-r border-white/5 flex flex-col h-screen fixed left-0 top-0 z-30 p-6 backdrop-blur-xl">
      {/* Brand Header */}
      <div className="flex items-center justify-between mb-2 cursor-pointer select-none" onClick={() => navigate('/')}>
        <span className="text-2xl font-black tracking-[0.075em] bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-blue-400 font-display">
          ZePlay
        </span>
        {/* Plan badge */}
        <button
          onClick={(e) => { e.stopPropagation(); navigate('/subscription'); }}
          className={`inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded transition-all hover:opacity-90 active:scale-95 ${
            planName === 'premium'
      ...
      {/* Footer */}
      <div className="pt-4 border-t border-white/5 text-center text-[10px] text-neutral-500 font-sans space-y-1 select-none">
        ...
        <div>
          <a 
            ...
            className="text-brand-accent hover:underline font-bold tracking-wider hover:text-blue-450 transition-colors"
          >
