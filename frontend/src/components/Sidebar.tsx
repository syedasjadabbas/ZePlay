...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      {/* Desktop Sidebar (md+) */}
      <aside role="navigation" aria-label="Main navigation" className="hidden md:flex fixed left-0 top-0 bottom-0 w-56 bg-[#000000] flex-col justify-between z-30 py-8 px-6">
        {/* Brand */}
      ...
      <nav role="navigation" aria-label="Mobile navigation" className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#080c14]/95 backdrop-blur-xl border-t border-white/10 z-40 flex items-center justify-around px-2 py-1 select-none">
        <button onClick={handleLogoClick} className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${isActive('/') ? 'text-brand-accent' : 'text-neutral-400'}`}>
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isActive('/') ? 2.5 : 1.75}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
          </svg>
          <span>Home</span>
        </button>

        <button onClick={() => navigate('/browse')} className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${isActive('/browse') ? 'text-brand-accent' : 'text-neutral-400'}`}>
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isActive('/browse') ? 2.5 : 1.75}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <span>Browse</span>
        </button>

        <button onClick={() => navigate('/my-list')} className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${isActive('/my-list') ? 'text-brand-accent' : 'text-neutral-400'}`}>
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isActive('/my-list') ? 2.5 : 1.75}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
          </svg>
          <span>My List</span>
        </button>

        <button onClick={() => navigate('/history')} className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${isActive('/history') ? 'text-brand-accent' : 'text-neutral-400'}`}>
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isActive('/history') ? 2.5 : 1.75}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <span>History</span>
        </button>
        ...
        {isAdmin ? (
          <button onClick={() => navigate('/admin/upload')} className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${isActive('/admin/upload') ? 'text-amber-400' : 'text-amber-700/80'}`}>
          ...
          <button onClick={() => navigate('/settings')} className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${isActive('/settings') ? 'text-brand-accent' : 'text-neutral-400'}`}>
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isActive('/settings') ? 2.5 : 1.75}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
            </svg>
            <span>Settings</span>
          </button>
