...
const ForgotPassword: React.FC = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage("If a matching account exists, a reset link has been sent. Check your local logs or inbox.");
    } catch (err: any) {
      setError(
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative">
        ...
        <div className="w-full max-w-md bg-[#0B1535]/80 border border-white/5 rounded-2xl p-8 md:p-10 shadow-2xl backdrop-blur-md relative z-10">
          <div className="space-y-2 mb-8">
            ...
            <p className="text-xs text-brand-textMuted font-medium">Enter your registered email to request a reset link.</p>
          </div>
          ...
          {!successMessage ? (
            ...
            <div className="pt-2">
              <button 
                ...
                className="w-full py-3 bg-brand-surface hover:bg-brand-cards text-white border border-white/5 font-bold rounded-xl transition-all text-sm"
              >
                Return to Login
              </button>
            </div>
